...
class DocumentationGenerator {
     ...
     */
    async generateDocumentation(parsedFeature, options) {
        let content;
        let filePath;
     ...
     */
    async generateDocumentationForFeatures(parsedFeatures, options) {
        const docs = [];
     ...
     */
    generateMarkdownDocumentation(parsedFeature, options) {
        ...
        const passedScenarios = feature.scenarios.filter(s => this.executionResults.get(this.getScenarioId(s))?.status === 'passed').length;
     ...
     */
    generateHtmlDocumentation(parsedFeature, options) {
...
<body>
     ...
     */
    generateMdxDocumentation(parsedFeature, options) {
        const { feature } = parsedFeature;
        let content = '';
     ...
     */
    generateMermaidDiagram(feature) {
     ...
     * Generate file paths
     */
    generateMarkdownFilePath(parsedFeature) {
    ...
    generateHtmlFilePath(parsedFeature) {
        const sanitizedName = parsedFeature.feature.name
            .toLowerCase()
            .replace(/[^a-z0-9\s]/g, '')
            .replace(/\s+/g, '_');
    ...
    generateMdxFilePath(parsedFeature) {
        const sanitizedName = parsedFeature.feature.name
            .toLowerCase()
            .replace(/[^a-z0-9\s]/g, '')
            .replace(/\s+/g, '_');
